...
class Crud_bpupengeluarankas extends CI_Model {
	//--------base---------------
	...
	function get_all(){
		return $this->db->get('bpupengeluarankas')->result_array();
	}

	function get_row(){
		return $this->db->get('bpupengeluarankas')->row_array();
	}

	function posts($data){
		return $this->db->insert('bpupengeluarankas', $data);
	}

	function puts($data){
		return $this->db->update('bpupengeluarankas', $data);
	}

	function delete($data){
		return $this->db->delete('bpupengeluarankas', $data);
	}
	...
	function get_option_child() {
		$res = $this->where('akun_void = 0')->where('akun_parent != 0')->get_all();
		$data = array();
		foreach ($res as $key => $value) {
			$data[] = array(
