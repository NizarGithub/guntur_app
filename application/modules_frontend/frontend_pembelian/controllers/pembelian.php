...
class Pembelian extends MY_Frontend
{
    ...
    private function do_add_permintaan()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            foreach ($barang as $key => $val) {
                $res = $this->db->query('SELECT brjs_harga_satuan FROM gntrapp_barang_jasa WHERE brjs_id = ' . $val)->row_array();
                $data = array(
                    'pbptnd_nopermintaan' => $this->input->post('pbptn_no'),
                    'pbptnd_jenisbarang' => $val,
                    'pbptnd_jumlah' => $jumlah[$key],
                    'pbptnd_hargabrg' => (!empty($res['brjs_harga_satuan']) ? $res['brjs_harga_satuan'] : 0),
                );
    ...
    private function total_permintaan($id = '')
    {
        ...
        foreach ($details as $key => $value) {
            ...
            $total[$value['pbptn_id']] += $value['pbptnd_jumlah'] * $value['pbptnd_hargabrg'];
        }
